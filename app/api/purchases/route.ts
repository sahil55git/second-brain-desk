// Purchase bills (erp-architecture-plan.md, Phase 2). Like Sales but the
// bill number comes from the SUPPLIER (we don't allocate it), so there's no
// counter to increment. GST is still computed server-side for our records.
import { NextRequest, NextResponse } from "next/server";
import { Prisma } from "@prisma/client";
import { prisma, safeDbCall } from "@/lib/db";
import { computeLine, computeTotals, gstTreatment, type GstLineInput } from "@/lib/gst";
...
export async function POST(req: NextRequest) {
  ...
  const { partyId, billNo, date, paymentMode, notes } = body;
    ...
    prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      return tx.purchaseBill.create({
        data: {
          ...
          paymentMode: normalizeMode(paymentMode, "CREDIT"),
          notes: notes || null,
          lineItems: {
